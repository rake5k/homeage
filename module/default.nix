...

{
  ...
  config = mkIf (cfg.file != { }) (mkMerge [
    ...
    (mkIf (cfg.installationType == "activation") {
      home = {
        ...
        extraBuilderCommands =
          let
            stateFile = pkgs.writeText "homeage-state.json" (
              builtins.toJSON (map (secret: secret) (builtins.attrValues cfg.file))
            );
          in
          ''
    ...
    (mkIf (cfg.installationType == "systemd") {
      ...
      home.activation.homeageCleanup =
        let
      ...
      systemd.user.services =
        let
          mkServices = lib.attrsets.mapAttrs' (
            name: value:
            lib.attrsets.nameValuePair "${name}-secret" {
              Unit = {
